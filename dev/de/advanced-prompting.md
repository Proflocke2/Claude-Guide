...
> **Internes Anthropic-Testing:** XML-strukturierte Prompts liefern **nachweislich konsistentere Outputs *(bis zu 40% Halluzinationsreduktion — Community-Erfahrungswert, nicht offiziell belegt)* *(→ [Prompting best practices](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices))*** als unstrukturierte Äquivalente.
...
> **Layout-Regel:** Dokumente **oben** im Prompt, Query und Instructions **unten**. Bis zu **30% Qualitätsverbesserung** bei langen Kontexten durch diese Anordnung *(Community-Erfahrungswert, nicht offiziell belegt)*.



## 7. Das Think-Tool (Agentische Workflows)

Nicht zu verwechseln mit Extended Thinking — das `think`-Tool gibt Claude einen Scratchpad mitten im Tool-Loop, um vor dem nächsten Schritt kurz zu reflektieren.

```python
tools = [
    {
        "name": "think",
        "description": "Nutze das, um über bisher gesammelte Informationen nachzudenken und den nächsten Schritt zu entscheiden. Nicht für finale Antworten verwenden.",
        "input_schema": {
            "type": "object",
            "properties": {
                "thought": {"type": "string"}
    ...
    # ... deine eigentlichen Tools
]
```

Laut Anthropics Engineering-Post hat ein `think`-Tool zum SWE-bench-Ergebnis von 0,623 für Claude 3.7 Sonnet beigetragen — mit messbaren Verbesserungen bei richtlinienintensiven Customer-Service-Aufgaben in Kombination mit optimiertem Prompting.
